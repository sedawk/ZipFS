...
namespace util {
	...
	namespace filesystem {
		...
		template <typename T>
		inline std::basic_string<T> dirname(const std::basic_string<T>& path, const T delim) {
			auto position = path.find_last_of(delim, path.size() - 2);
			return (position == std::string::npos) ? std::basic_string<T>() : path.substr(0, position + 1);
		}

		template <typename T>
		inline std::basic_string<T> dirname(const T* path, const T delim) {
			return dirname(std::basic_string<T>(path), delim);
		}

		template <typename T>
		inline std::basic_string<T> filename(const std::basic_string<T>& path, const T delim) {
			auto position = path.find_last_of(delim, path.size() - 2);
			return (position == std::string::npos) ? path : path.substr(position + 1);
		}

		template <typename T>
		inline std::basic_string<T> filename(const T* path, const T delim) {
			return filename(std::basic_string<T>(path), delim);
		}
